...
<div align="center">
  ...
  <!-- NPM version -->
  <a href="https://npmjs.org/package/insight-api-neobytes">
    <img src="https://img.shields.io/npm/v/insight-api-neobytes.svg" alt="NPM version" />
  </a>
</div>


This is a backend-only service. If you're looking for the web frontend application, take a look at https://github.com/neobytes-project/insight-ui-neobytes.
...
- [Resources](#resources)
- [License](https://github.com/neobytes-project/insight-api-neobytes/blob/master/LICENSE)

## Getting Started

```bash
npm install -g neobytescore-node@latest
neobytescore-node create mynode
cd mynode
neobytescore-node install insight-api-neobytes
neobytescore-node start
```

The API endpoints will be available by default at: `http://localhost:3001/insight-api-neobytes/`

### Prerequisites

- [Neobytescore Node 3.x](https://github.com/neobytes-project/neobytescore-node)

**Note:** You can use an existing Neobytes data directory, however `txindex`, `addressindex`, `timestampindex` and `spentindex` needs to be set to true in `neobytes.conf`, as well as a few other additional fields.

### Query Rate Limit

To protect the server, insight-api has a built it query rate limiter. It can be configurable in `neobytescore-node.json` with:
``` json
  "servicesConfig": {
    "insight-api": {
      "rateLimiterOptions": {
...
```
With all the configuration options available: https://github.com/neobytes-project/insight-api-neobytes/blob/master/lib/ratelimiter.js#L10-17
...
### Block
```
  /insight-api-neobytes/block/[:hash]
  /insight-api-neobytes/block/0000000006e7b38e8ab2d351239019c01de9a148b5baef58cfe52dfd9917cedc
```

### Block Index
Get block hash by height
```
  /insight-api-neobytes/block-index/[:height]
  /insight-api-neobytes/block-index/0
```
This would return:
```
{
...
### Raw Block
```
  /insight-api-neobytes/rawblock/[:blockHash]
```
...
```
  /insight-api-neobytes/blocks?limit=3&blockDate=2017-04-22
```
...
### Transaction
```
  /insight-api-neobytes/tx/[:txid]
  /insight-api-neobytes/tx/ebdca263fe1c75c8609ce8fe3d82a320a0b3ca840f4df995883f5dab1b9ff8d9
  /insight-api-neobytes/rawtx/[:rawid]
  /insight-api-neobytes/rawtx/ebdca263fe1c75c8609ce8fe3d82a320a0b3ca840f4df995883f5dab1b9ff8d9
```

### Address
```
  /insight-api-neobytes/addr/[:addr][?noTxList=1][&from=&to=]
  /insight-api-neobytes/addr/ybi3gej7Ea1MysEYLR7UMs3rMuLJH5aVsW?noTxList=1
  /insight-api-neobytes/addr/yPv7h2i8v3dJjfSH4L3x91JSJszjdbsJJA?from=1000&to=2000
```

### Address Properties
```
  /insight-api-neobytes/addr/[:addr]/balance
  /insight-api-neobytes/addr/[:addr]/totalReceived
  /insight-api-neobytes/addr/[:addr]/totalSent
  /insight-api-neobytes/addr/[:addr]/unconfirmedBalance
```
The response contains the value in Satoshis.

### Unspent Outputs
```
  /insight-api-neobytes/addr/[:addr]/utxo
```
Sample return:
```
[
...
GET method:
```
  /insight-api-neobytes/addrs/[:addrs]/utxo
  /insight-api-neobytes/addrs/ygwNQgE5f15Ygopbs2KPRYMS4TcffqBpsz,ygw5yCtVkx3hREke4L8qDqQtnNoAiPKTSx/utxo
```

POST method:
```
  /insight-api-neobytes/addrs/utxo
```
...
```
  /insight-api-neobytes/txs/?block=HASH
  /insight-api-neobytes/txs/?block=000000000814dd7cf470bd835334ea6624ebf0291ea857a5ab37c65592726375
```
### Transactions by Address
```
  /insight-api-neobytes/txs/?address=ADDR
  /insight-api-neobytes/txs/?address=yWFfdp9nLUjy1kJczFhRuBMUjtTkTTiyMv
```

### Transactions for Multiple Addresses
GET method:
```
  /insight-api-neobytes/addrs/[:addrs]/txs[?from=&to=]
  /insight-api-neobytes/addrs/ygwNQgE5f15Ygopbs2KPRYMS4TcffqBpsz,ygw5yCtVkx3hREke4L8qDqQtnNoAiPKTSx/txs?from=0&to=20
```

POST method:
```
  /insight-api-neobytes/addrs/txs
```
...
POST method:
```
  /insight-api-neobytes/tx/sendix
```
POST params:
```
  rawtx: "signed transaction as hex string"
...
GET method:
```
  /insight-api-neobytes/gobject/list/proposal
```
...
GET method:
```
  /insight-api-neobytes/gobject/get/[:hash]
  /insight-api-neobytes/gobject/get/b6af3e70c686f660541a77bc035df2e5e46841020699ce3ec8fad786f7d1aa35
```
...
### Masternodes List
```
  /insight-api-neobytes/masternodes/list
```
### Validate Masternode
```
  /insight-api-neobytes/masternodes/validate/[:payee]
  /insight-api-neobytes/masternodes/validate/yRuALkPpeYpTgxdNn2L5YgGktASJYDYPAo
```
...
```
  /insight-api-neobytes/sync
```

### Live Network P2P Data Sync Status
```
  /insight-api-neobytes/peer
```

### Status of the Bitcoin Network
```
  /insight-api-neobytes/status?q=xxx
```
...
### Utility Methods
```
  /insight-api-neobytes/utils/estimatefee[?nbBlocks=2]
```
...
- (Medium)[How to setup a Neobytes Instant-Send Transaction using Insight API?????????The comprehensive way](https://medium.com/@obusco/setup-instant-send-transaction-the-comprehensive-way-a80a8a0572e)
